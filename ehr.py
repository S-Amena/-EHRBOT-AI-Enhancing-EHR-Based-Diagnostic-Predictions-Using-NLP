...
def query_chatbot(llm, user_input, tavily_client, chat_id):
    ...
    try:
        ...
        messages = [
            ChatMessage(
                role="system", 
                content="You are an expert Helthcare AI Assitant chatbot. Use the provided contexts in your response."
            ),
...
def chatbot_interface():
    ...
    # Handle new messages
    user_input = st.chat_input("Ask your Healthcare related query...")
    if user_input:
        try:
...
def signup_page(authenticator):
    hide_sidebar()
    st.title("🩺 AI Healthcare Chatbo - Signup")
...
def fallback_response(llm, user_input):
    ...
    try:
        ...
        messages = [
            ChatMessage(
                role="system",
                content="You are an expert Healthcare AI Assistant. Provide guidance and advice."
            ),
